...
func (d *DB) GetBlockers(taskID string) ([]Task, error) {
	rows, err := d.q.Query(
		`SELECT t.id, t.title, t.description, t.status, t.block_reason, t.assignee, t.parent_id, t.repo, t.base_branch, t.created_at, t.updated_at
		 FROM tasks t
...
func scanTasks(rows *sql.Rows) ([]Task, error) {
	...
	for rows.Next() {
		var t Task
		if err := rows.Scan(&t.ID, &t.Title, &t.Description, &t.Status, &t.BlockReason, &t.Assignee, &t.ParentID, &t.Repo, &t.BaseBranch, &t.CreatedAt, &t.UpdatedAt); err != nil {
